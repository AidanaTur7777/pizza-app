...
import {
  IPizzaItemSlice,
  IPizzaItemSliceArgs,
  IPizzaItemSliceBlock,
} from '../../@types/pizzaItemSliceTypes'
...
export const fetchPizza = createAsyncThunk<
  IPizzaItemSliceBlock,
  IPizzaItemSliceArgs
>('pizza/fetchPizza', async ({ currentLanguage, id }) => {
  const response = await axios.get<IPizzaItemSliceBlock>(
    `https://62c007ccd40d6ec55cc99bf2.mockapi.io/Items${currentLanguage}/` + id
  )
...
export const pizzaItemSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<IPizzaItemSliceBlock>) => {
